...
const initPlayer = (config) => {
    ...
    // SFX for countdown
    const sfx = new Audio((typeof AMBIENT_SOUNDS !== 'undefined' ? AMBIENT_SOUNDS?.countdown : null) || 'https://cdn.for-you-always.my.id/1772227895645-j1jcgl.mp3?v=3');
    sfx.volume = 0.3;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!giftIdParam && window.STANDALONE_CONFIG) {
        ...
        if (giftIdParam === 'for-preview') {
            ...
            if (!giftConfig || !giftConfig.photos || giftConfig.photos.length === 0) handleAfterLoad();
        } else {
            showState('loading');

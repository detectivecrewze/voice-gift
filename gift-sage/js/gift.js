...
const _fetchAndRender = async (giftId) => {
  ...
  try {
    ...
    const endpoint = `${API_BASE_URL}/get-config?id=${giftId}${cacheBuster}&t=${Date.now()}`;
...
const _setupPasswordGate = (giftId, partialGift) => {
  ...
  const tryUnlock = async () => {
    ...
    try {
      ...
      const urlParams = new URLSearchParams(window.location.search);
      const cacheBuster = urlParams.get('t') ? `&t=${urlParams.get('t')}` : '';
      const response = await fetch(`${API_BASE_URL}/get-config?id=${giftId}${cacheBuster}&t=${Date.now()}`, { signal: controller.signal });
      clearTimeout(timeoutId);
